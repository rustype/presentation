...
\begin{frame}[fragile]
    ...
    \begin{figure}
        ...
        \begin{tikzpicture}
            ...
            \draw[transition] (init) edge[out=-15, in=-165] node[below, font=\tiny\ttfamily] {open:ERROR} (end);
            \draw[transition] (open) -- node[above, font=\tiny\ttfamily] {read} (close);
...
fn main() {
    ...
    builder.build();
         // ^^^^^
         // | error: you cannot buildlabel without setting `y`
}
        \end{minted}
    \end{listing}
